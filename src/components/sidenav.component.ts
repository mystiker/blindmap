...
export class SidenavComponent extends HTMLElement {
  ...
  private onAppStateChange(state: AppState) {
    this.updatePosition(state.position);
    this.updateSymbol(state);
  }

  private updatePosition(position: AppState["position"]) {
  ...
  private updateSymbol(state: AppState) {
    if (this.symbolInput === null) {
      console.warn("Could not update Symbol: Symbol Input Element not found");
      return;
    }

    const room = state.loadedLabyrinth.rooms?.find(
      (room) =>
        room.x === state.position.x &&
